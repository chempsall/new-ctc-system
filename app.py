...
def _ensure_special_projects(c, now):
...
def _get_bank_holidays(c, period_start):
...
def _nightly_fetch_bank_holidays(c, now):
    try:
...
def _nightly_special_rtcs():
    ...
    cutoff_period  = (today - _rd(months=12)).replace(day=1).isoformat()
    future_end     = (today + _rd(months=11)).replace(day=1)

    _nightly_fetch_bank_holidays(c, now)
    _ensure_special_projects(c, now)
        ...
        for cfg in SPECIAL_RTC_CONFIGS:
            ...
            for sr in staff_rows:
                ...
                for period in period_list:
                    ...
                    if not existing:
                        days = 0
                        if cfg["has_bank_holidays"]:
                            days = _get_bank_holidays(c, period)
                        c.execute("""
...
@app.route("/admin/special-rtcs", methods=["POST"])
@require_admin
